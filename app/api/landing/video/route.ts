...
const FIDELIDAD = " CRITICAL: keep the product EXACTLY as in the reference image — same real proportions, dimensions, thickness and scale. Do NOT stretch, squash, widen, elongate, bend, warp or resize the product. Same shape, colors, logos, text and details. Only animate the movement; the product itself must stay rigid and unchanged. If MORE THAN ONE unit of the product appears in the scene, EVERY single unit must be an identical exact copy of the reference product — same model, same design, same colors, same details on all of them; never invent different versions, variations, other models or mismatched units.";
