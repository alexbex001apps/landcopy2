...
export default function Copy() {
  ...
  const paises = [
    { nombre: "Colombia", flag: "­ƒç¿­ƒç┤" },
    { nombre: "M├®xico", flag: "­ƒç▓­ƒç¢" },
    { nombre: "Venezuela", flag: "­ƒç╗­ƒç¬" },
    { nombre: "Costa Rica", flag: "­ƒç¿­ƒçÀ" },
    { nombre: "Ecuador", flag: "­ƒç¬­ƒç¿" },
    { nombre: "General", flag: "­ƒîÄ" },
  ];
  ...
  const productosLluvia = ["­ƒôª","­ƒøì´©Å","­ƒÆè","­ƒÆ╗","­ƒÅï´©Å","­ƒì│","­ƒÆä","ÔÜí","­ƒº┤","­ƒÄ«","­ƒî┐","­ƒöï","­ƒÑÿ","­ƒÆà","­ƒ¬┤","­ƒ®║","­ƒô▒","­ƒº¼","­ƒÅà","­ƒºÿ"];
 
  const frasesGenerando = [
    "Tu competencia no est├í haciendo esto ahora mismo...",
    "Construyendo copy que vende mientras esperas...",
    "La IA est├í analizando tu mercado objetivo...",
    "Generando 7 d├¡as de campa├▒a completa...",
    "Cada palabra est├í siendo dise├▒ada para convertir...",
    "Esto tomar├¡a 2-3 d├¡as a un copywriter profesional...",
  ];
  ...
  async function analizarCompetidor() {
    ...
    try {
      ...
      if (!res.ok || data.error) {
        setErrorAnalisis(data.error || "No pudimos analizar esa p├ígina.");
      } else {
        setAnalisisCompetidor(data);
      }
    } catch {
      setErrorAnalisis("No pudimos conectar con esa p├ígina. Verifica la URL.");
    }
    setAnalizando(false);
  }
      ...
      <div className="max-w-[1400px] mx-auto px-6 pt-24 pb-20">
        ...
        <div className="text-center mb-10">
          <div className="inline-flex items-center gap-2 bg-orange-500/10 border border-orange-500/30 text-orange-500 text-xs font-semibold px-4 py-1.5 rounded-full mb-4">
            IA generativa ┬À copy profesional
          </div>
          ...
          <p className="text-zinc-500 text-base">Sube tu producto ÔÇö la IA genera landing, WhatsApp, Meta Ads, redes y campa├▒a de 7 d├¡as en segundos</p>
        </div>
        ...
        <div className="grid grid-cols-[380px_1fr] gap-6">
          ...
          <div className="bg-[#0a0a0a] border border-[#1a1a1a] rounded-2xl p-5">
            <div className="flex items-center gap-2 mb-5 pb-3 border-b border-[#151515]">
              <span className="text-orange-500 text-lg">­ƒôª</span>
              ...
              <div onClick={() => !imagen && fileRef.current?.click()} className="border border-dashed border-[#222] rounded-xl p-6 text-center cursor-pointer hover:border-orange-500 transition-colors relative">
                {imagen ? (
                  <div className="relative inline-block">
                    ...
                    <button onClick={quitarImagen} className="absolute -top-2 -right-2 bg-red-500 hover:bg-red-600 text-white text-[10px] font-bold w-5 h-5 rounded-full flex items-center justify-center">Ô£ò</button>
                  </div>
                ) : (
                  <>
                    <div className="text-orange-500 text-5xl mb-3">­ƒôÀ</div>
                    <div className="text-[#f0ead6] text-xs font-semibold mb-1">Arrastra o selecciona ┬À JPG PNG WEBP</div>
            ...
            <div className="mb-3">
              <label className="text-orange-500 text-[10px] font-bold tracking-widest uppercase mb-1.5 block">Caracter├¡sticas ÔÇö Amazon ┬À MeLi ┬À tu tienda</label>
              <textarea value={caracteristicas} onChange={e => setCaracteristicas(e.target.value)} placeholder="Pega aqu├¡ ┬À la IA convierte cada caracter├¡stica en beneficio emocional..." className="w-full bg-[#f0ead6] border border-[#d4cdb8] text-[#1a1a1a] rounded-lg px-3 py-2 text-xs outline-none placeholder-[#999] resize-none h-16" />
              <p className="text-orange-500 text-[10px] mt-1 font-medium">La IA detecta el formato y lo procesa autom├íticamente</p>
            </div>
            ...
            <div className="mb-3">
              ...
              {analizando && (
                <div className="mt-3 bg-[#0d0d0d] border border-[#1e2a3a] rounded-xl p-4 flex flex-col gap-3">
                  <div className="text-cyan-400 text-[10px] font-bold tracking-widest uppercase mb-1">­ƒöì Analizando competidor...</div>
                  {[
                    { icon: "­ƒîÉ", texto: "Leyendo p├ígina del competidor", color: "bg-cyan-400", w: "100%", pts: null },
                    { icon: "­ƒôØ", texto: "TITULAR ÔÇö beneficio claro, urgencia, voz del cliente", color: "bg-orange-500", w: "75%", pts: "25 pts" },
                    { icon: "­ƒÆí", texto: "PROPUESTA ÔÇö diferenciaci├│n, dolor, credibilidad", color: "bg-purple-500", w: "55%", pts: "25 pts" },
                    { icon: "Ô¡É", texto: "PRUEBA SOCIAL ÔÇö testimonios, n├║meros, resultados", color: "bg-yellow-400", w: "35%", pts: "25 pts" },
                    { icon: "ÔÜí", texto: "URGENCIA Y CTA ÔÇö escasez, acci├│n, raz├│n para hoy", color: "bg-green-400", w: "15%", pts: "25 pts" },
                  ].map((p, i) => (
              ...
              {errorAnalisis && !analizando && (
                <div className="mt-3 bg-[#130808] border border-[#7f1d1d] rounded-xl p-4 flex gap-3">
                  <span className="text-red-400 text-lg mt-0.5">ÔÜá</span>
                  <div>
                    <div className="text-red-400 text-[11px] font-bold mb-1">No pudimos analizar esa p├ígina</div>
                    <div className="text-[#f0ead6] text-[11px] leading-relaxed">{errorAnalisis}</div>
                    <button onClick={() => setErrorAnalisis(null)} className="mt-2 text-cyan-400 border border-cyan-400 rounded-md px-3 py-1 text-[10px] font-bold">Ôå╗ Intentar con otra URL</button>
                  </div>
                </div>
              )}
              ...
              {analisisCompetidor && !analizando && (
                <div className="mt-3 bg-[#0d0d0d] border border-[#1e3a2e] rounded-xl overflow-hidden">
                  <div className="flex items-center justify-between px-4 py-2.5 border-b border-[#1a1a1a]">
                    <span className="text-cyan-400 text-[10px] font-bold tracking-widest uppercase">AN├üLISIS DEL COMPETIDOR</span>
                  ...
                  {analisisCompetidor.puntos?.map((p: any, i: number) => (
                    <div key={i} className="grid grid-cols-2 border-b border-[#111] last:border-none">
                      <div className="p-3 bg-[#130808] border-r border-[#1a1a1a]">
                        <div className="text-red-400 text-[10px] font-black mb-1">Ô£ò {p.categoria}</div>
                        <div className="text-[#f0ead6] text-[11px] leading-relaxed">{p.ellos}</div>
                      </div>
                      <div className="p-3 bg-[#081308]">
                        <div className="text-green-400 text-[10px] font-black mb-1">Ô£ô {p.categoria}</div>
                  ...
                  <div className="flex gap-2 px-4 py-3 border-t border-[#1a1a1a]">
                    <button onClick={() => setAnalisisActivo(true)} className="flex-1 bg-orange-500 text-white font-bold py-2 rounded-lg text-xs">ÔÜí Usar este an├ílisis al generar</button>
              ...
              {analisisActivo && !analisisCompetidor && (
                <div className="mt-3 bg-[#081308] border border-[#166534] rounded-lg px-3 py-2.5 flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    ...
                    <div>
                      <div className="text-green-400 text-[11px] font-bold">An├ílisis del competidor activo</div>
                      <div className="text-[#f0ead6] text-[10px]">{competidor}</div>
                    </div>
                  </div>
                  <button onClick={() => setAnalisisActivo(false)} className="text-red-400 border border-red-400 rounded-md px-2 py-1 text-[10px] font-bold">Ô£ò Quitar</button>
                </div>
              )}
            </div>
            ...
            <div className="mb-3">
              <label className="text-orange-500 text-[10px] font-bold tracking-widest uppercase mb-1.5 block">Pa├¡s objetivo</label>
            ...
            <div className="mb-3">
              <label className="text-orange-500 text-[10px] font-bold tracking-widest uppercase mb-1.5 block">Tono de comunicaci├│n</label>
              <div className="flex flex-wrap gap-1.5">
                {tonos.map(t => (
            ...
            <div className="mb-4">
              <label className="text-orange-500 text-[10px] font-bold tracking-widest uppercase mb-1.5 block">Categor├¡a</label>
              <select value={categoria} onChange={e => setCategoria(e.target.value)} className="w-full bg-[#f0ead6] border border-[#d4cdb8] text-[#1a1a1a] rounded-lg px-3 py-2 text-xs h-[34px] outline-none">
                {["Salud y bienestar","Hogar","Tecnolog├¡a","Belleza","Deporte","Cocina","Moda","Mascotas","Otro"].map(c => <option key={c}>{c}</option>)}
              </select>
            </div>
            ...
            <div className="mb-4">
              <label className="text-orange-500 text-[10px] font-bold tracking-widest uppercase mb-2 block">Qu├® generar</label>
              <div className="grid grid-cols-2 gap-1.5">
                {["Landing page","WhatsApp x3","Meta Ads","Redes sociales","Campa├▒a 7 d├¡as","Email seguimiento","Objeciones","SEO + extras"].map(item => (
            ...
            <button onClick={limpiarTodo} className="w-full bg-[#0d0d0d] border border-red-500/30 text-red-400 font-bold py-2 rounded-xl text-xs mb-2 transition-colors flex items-center justify-center gap-2">
              ­ƒùæ´©Å Limpiar todo y empezar de nuevo
            </button>
            ...
            <button onClick={() => generar(true)} disabled={!producto || !imagen || loading} className="w-full bg-[#050505] border border-cyan-400/35 text-cyan-400 font-bold py-2.5 rounded-xl text-xs transition-colors flex items-center justify-center gap-2 disabled:opacity-40">
              ­ƒæü´©Å Generar con an├ílisis de imagen ┬À GPT-4o Vision
            </button>
          </div>
          ...
          <div className="bg-[#0a0a0a] border border-[#1a1a1a] rounded-2xl p-5">
            ...
            {resultado && (
              <div className="bg-[#0d0d0d] border border-[#1a1a1a] rounded-xl px-4 py-3 mb-4 flex items-center justify-between">
                <div className="flex items-center gap-3">
                  <span className="text-orange-500 text-lg">ÔÜí</span>
                  <div className="text-xs text-zinc-400 leading-5">
                    Generaste en <span className="text-white font-bold">{tiempoReal} segundos</span> lo que un copywriter har├¡a en <span className="text-white font-bold">2-3 d├¡as</span><br/>
            ...
            {resultado && (
              <div className="grid grid-cols-4 gap-2 mb-4">
                ...
                <div className="bg-[#0a0a0a] border border-[#151515] rounded-xl p-3 text-center">
                  ...
                  <div className="text-zinc-500 text-[10px]">Pa├¡s</div>
                </div>
              </div>
            )}
            ...
            <div className="flex gap-1 mb-4 bg-[#070707] border border-[#111] rounded-xl p-1">
              {[
                ...
                {id:"campana",label:"Campa├▒a",color:"bg-green-400"},
            ...
            <button onClick={() => resultado && navigator.clipboard.writeText(Object.values(resultado).join("\n\n"))} className="w-full bg-[#0d0d0d] border border-[#1e1e1e] text-[#f0ead6] rounded-lg py-2 text-xs mb-4 flex items-center justify-center gap-2">
              ­ƒôï Copiar todo el contenido generado
            </button>
            ...
            {loading && (
              <div className="bg-[#070707] border border-[#111] rounded-xl p-4 mb-4 relative overflow-hidden" style={{minHeight: "300px"}}>
                ...
                <div className="relative z-10">
                  <div className="text-orange-500 text-[10px] font-bold tracking-widest uppercase mb-3">ÔÜÖ´©Å Generando contenido ┬À {progreso}%</div>
                  {[
                    {icon:"­ƒºá",texto:"Analizando producto y mercado",color:"bg-orange-500"},
                    {icon:"­ƒÄ»",texto:"Identificando puntos de dolor",color:"bg-cyan-400"},
                    {icon:"Ô£ì´©Å",texto:"Escribiendo copy persuasivo",color:"bg-purple-500"},
                    {icon:"­ƒÜÇ",texto:"Optimizando campa├▒a de 7 d├¡as",color:"bg-green-400"},
                  ].map((p,i) => (
            ...
            {resultado && (
              <div className="space-y-4">
                ...
                {tabActivo === "landing" && (
                  <>
                    {[
                      {key:"hero",titulo:"Hero ÔÇö titular"},
                      {key:"problema",titulo:"El problema"},
                      {key:"solucion",titulo:"La soluci├│n"},
                      ...
                      <div key={s.key} className="border border-orange-500 rounded-xl overflow-hidden">
                        <div className="flex items-center justify-between px-4 py-2.5 border-b border-orange-500/20">
                          ...
                          <div className="flex gap-1.5">
                            <button disabled={seccionCargando !== null} onClick={() => regenerar(s.key)} className="bg-orange-500/10 border border-orange-500/25 text-orange-500 text-[10px] font-bold px-2 py-1 rounded-md disabled:opacity-50">{seccionCargando === `regenerar-${s.key}` ? "ÔÅ│ Generando..." : "Ôå╗ Regenerar"}</button>
                            <button disabled={seccionCargando !== null} onClick={() => mejorar(s.key)} className="bg-cyan-400/10 border border-cyan-400/20 text-cyan-400 text-[10px] font-bold px-2 py-1 rounded-md disabled:opacity-50">{seccionCargando === `mejorar-${s.key}` ? "ÔÅ│ Mejorando..." : "Ôåæ Mejorar"}</button>
                            <button onClick={() => copiar(resultado[s.key], s.key)} className="bg-[#111] border border-[#1e1e1e] text-zinc-400 text-[10px] font-bold px-2 py-1 rounded-md">{copiado === s.key ? "Ô£ô Copiado" : "Copiar"}</button>
                            <button onClick={() => guardar(resultado[s.key], s.titulo)} className="bg-green-400/10 border border-green-400/20 text-green-400 text-[10px] font-bold px-2 py-1 rounded-md">ÔØñ Guardar</button>
                          </div>
                        </div>
                ...
                {tabActivo === "whatsapp" && resultado.whatsapp && (
                  <div className="border border-cyan-400 rounded-xl overflow-hidden">
                    <div className="flex items-center justify-between px-4 py-2.5 border-b border-cyan-400/20">
                      <span className="text-cyan-400 text-[10px] font-bold tracking-widest uppercase">WhatsApp ┬À 3 versiones</span>
                      <div className="flex gap-1.5">
                        <button disabled={seccionCargando !== null} onClick={() => regenerar("whatsapp")} className="bg-orange-500/10 border border-orange-500/25 text-orange-500 text-[10px] font-bold px-2 py-1 rounded-md disabled:opacity-50">{seccionCargando === "regenerar-whatsapp" ? "ÔÅ│ Generando..." : "Ôå╗ Regenerar"}</button>
                        <button disabled={seccionCargando !== null} onClick={() => mejorar("whatsapp")} className="bg-cyan-400/10 border border-cyan-400/20 text-cyan-400 text-[10px] font-bold px-2 py-1 rounded-md disabled:opacity-50">{seccionCargando === "mejorar-whatsapp" ? "ÔÅ│ Mejorando..." : "Ôåæ Mejorar"}</button>
                        <button onClick={() => copiar(resultado.whatsapp, "whatsapp")} className="bg-[#111] border border-[#1e1e1e] text-zinc-400 text-[10px] font-bold px-2 py-1 rounded-md">{copiado === "whatsapp" ? "Ô£ô Copiado" : "Copiar"}</button>
                        <button onClick={() => guardar(resultado.whatsapp,"WhatsApp x3")} className="bg-green-400/10 border border-green-400/20 text-green-400 text-[10px] font-bold px-2 py-1 rounded-md">ÔØñ Guardar</button>
                      </div>
                    </div>
                ...
                {tabActivo === "campana" && resultado.campana && (() => {
                  const parsearDias = (texto: string) => {
                    ...
                    partes.forEach((parte, i) => {
                      ...
                      const primeraLinea = lineas[0].replace(/^\*\*/g, "").replace(/\*\*$/g, "").replace(/^D[├¡i]a\s*\d+[:\-]?\s*/i, "").trim();
                      const resto = lineas.slice(1).join("\n").trim();
                      dias.push({ titulo: primeraLinea || `D├¡a ${i + 1}`, texto: resto || primeraLinea });
                    });
                    return dias;
                  };
                      ...
                      {dias.map((dia, i) => (
                        <div key={i} className="border border-green-400 rounded-xl overflow-hidden">
                          <div className="flex items-center justify-between px-4 py-2.5 border-b border-green-400/20">
                            <div className="flex items-center gap-2">
                              <span className="bg-orange-500 text-white text-[10px] font-black px-2.5 py-1 rounded-md">D├ìA {i + 1}</span>
                            ...
                            <div className="flex gap-1.5">
                              <button disabled={seccionCargando !== null} onClick={async () => {
                                ...
                                const data = await res.json();
                                if (data.campana_dia) { const nuevosDias = [...dias]; nuevosDias[i] = data.campana_dia; setResultado((prev: any) => ({ ...prev, campana: nuevosDias.map((d, idx) => `D├¡a ${idx + 1}: ${d.titulo}\n${d.texto}`).join("\n") })); }
                                setSeccionCargando(null);
                              }} className="bg-orange-500/10 border border-orange-500/25 text-orange-500 text-[10px] font-bold px-2 py-1 rounded-md disabled:opacity-50">{seccionCargando === `regenerar-campana-dia-${i + 1}` ? "ÔÅ│ Generando..." : "Ôå╗ Regenerar"}</button>
                              <button disabled={seccionCargando !== null} onClick={async () => {
                                ...
                                const data = await res.json();
                                if (data.texto) { const nuevosDias = [...dias]; nuevosDias[i] = { titulo: dia.titulo, texto: data.texto }; setResultado((prev: any) => ({ ...prev, campana: nuevosDias.map((d, idx) => `D├¡a ${idx + 1}: ${d.titulo}\n${d.texto}`).join("\n") })); }
                                setSeccionCargando(null);
                              }} className="bg-cyan-400/10 border border-cyan-400/20 text-cyan-400 text-[10px] font-bold px-2 py-1 rounded-md disabled:opacity-50">{seccionCargando === `mejorar-campana-dia-${i + 1}` ? "ÔÅ│ Mejorando..." : "Ôåæ Mejorar"}</button>
                              <button onClick={() => copiar(`${dia.titulo}\n${dia.texto}`, `campana-dia-${i + 1}`)} className="bg-[#111] border border-[#1e1e1e] text-zinc-400 text-[10px] font-bold px-2 py-1 rounded-md">{copiado === `campana-dia-${i + 1}` ? "Ô£ô Copiado" : "Copiar"}</button>
                              <button onClick={() => guardar(`${dia.titulo}\n${dia.texto}`, `Campa├▒a D├¡a ${i + 1}`)} className="bg-green-400/10 border border-green-400/20 text-green-400 text-[10px] font-bold px-2 py-1 rounded-md">ÔØñ Guardar</button>
                            </div>
                          </div>
                ...
                {tabActivo === "prompts" && (
                  <div className="border border-purple-500 rounded-xl overflow-hidden">
                    <div className="flex items-center justify-between px-4 py-2.5 border-b border-purple-500/20">
                      <span className="text-purple-400 text-[10px] font-bold tracking-widest uppercase">Prompts para im├ígenes IA</span>
                      <div className="flex gap-1.5">
                        <button onClick={() => resultado?.prompts && copiar(resultado.prompts, "prompts")} className="bg-[#111] border border-[#1e1e1e] text-zinc-400 text-[10px] font-bold px-2 py-1 rounded-md">{copiado === "prompts" ? "Ô£ô Copiado" : "Copiar"}</button>
                      </div>
                    </div>
                    ...
                    <div className="px-4 py-3 border-t border-purple-500/20">
                      <button className="w-full bg-cyan-400/10 border border-cyan-400/30 text-cyan-400 rounded-lg py-2 text-xs font-bold">ÔåÆ Ir al m├│dulo de im├ígenes con este producto</button>
                    </div>
                  </div>
                )}
                ...
                {tabActivo === "metaads" && resultado.metaads && (
                  <div className="border border-pink-500 rounded-xl overflow-hidden">
                    <div className="flex items-center justify-between px-4 py-2.5 border-b border-pink-500/20">
                      <span className="text-pink-500 text-[10px] font-bold tracking-widest uppercase">Meta Ads ┬À 5 anuncios</span>
                      <div className="flex gap-1.5">
                        <button disabled={seccionCargando !== null} onClick={() => regenerar("metaads")} className="bg-orange-500/10 border border-orange-500/25 text-orange-500 text-[10px] font-bold px-2 py-1 rounded-md disabled:opacity-50">{seccionCargando === "regenerar-metaads" ? "ÔÅ│ Generando..." : "Ôå╗ Regenerar"}</button>
                        <button disabled={seccionCargando !== null} onClick={() => mejorar("metaads")} className="bg-cyan-400/10 border border-cyan-400/20 text-cyan-400 text-[10px] font-bold px-2 py-1 rounded-md disabled:opacity-50">{seccionCargando === "mejorar-metaads" ? "ÔÅ│ Mejorando..." : "Ôåæ Mejorar"}</button>
                        <button onClick={() => copiar(resultado.metaads, "metaads")} className="bg-[#111] border border-[#1e1e1e] text-zinc-400 text-[10px] font-bold px-2 py-1 rounded-md">{copiado === "metaads" ? "Ô£ô Copiado" : "Copiar"}</button>
                        <button onClick={() => guardar(resultado.metaads,"Meta Ads")} className="bg-green-400/10 border border-green-400/20 text-green-400 text-[10px] font-bold px-2 py-1 rounded-md">ÔØñ Guardar</button>
                      </div>
                    </div>
                ...
                {tabActivo === "extras" && (
                  <div className="space-y-4">
                    {resultado.seo && (
                      <div className="border border-zinc-600 rounded-xl overflow-hidden">
                        <div className="flex items-center justify-between px-4 py-2.5 border-b border-zinc-600/20">
                          <span className="text-zinc-400 text-[10px] font-bold tracking-widest uppercase">­ƒöì SEO Keywords</span>
                          <div className="flex gap-1.5">
                            <button disabled={seccionCargando !== null} onClick={() => regenerar("seo")} className="bg-orange-500/10 border border-orange-500/25 text-orange-500 text-[10px] font-bold px-2 py-1 rounded-md disabled:opacity-50">{seccionCargando === "regenerar-seo" ? "ÔÅ│ Generando..." : "Ôå╗ Regenerar"}</button>
                            <button disabled={seccionCargando !== null} onClick={() => mejorar("seo")} className="bg-cyan-400/10 border border-cyan-400/20 text-cyan-400 text-[10px] font-bold px-2 py-1 rounded-md disabled:opacity-50">{seccionCargando === "mejorar-seo" ? "ÔÅ│ Mejorando..." : "Ôåæ Mejorar"}</button>
                            <button onClick={() => copiar(resultado.seo, "seo")} className="bg-[#111] border border-[#1e1e1e] text-zinc-400 text-[10px] font-bold px-2 py-1 rounded-md">{copiado === "seo" ? "Ô£ô Copiado" : "Copiar"}</button>
                            <button onClick={() => guardar(resultado.seo, "SEO Keywords")} className="bg-green-400/10 border border-green-400/20 text-green-400 text-[10px] font-bold px-2 py-1 rounded-md">ÔØñ Guardar</button>
                          </div>
                        </div>
                    ...
                    {resultado.objeciones && (
                      <div className="border border-zinc-600 rounded-xl overflow-hidden">
                        <div className="flex items-center justify-between px-4 py-2.5 border-b border-zinc-600/20">
                          <span className="text-zinc-400 text-[10px] font-bold tracking-widest uppercase">­ƒÆ¼ Objeciones y Respuestas</span>
                          <div className="flex gap-1.5">
                            <button disabled={seccionCargando !== null} onClick={() => regenerar("objeciones")} className="bg-orange-500/10 border border-orange-500/25 text-orange-500 text-[10px] font-bold px-2 py-1 rounded-md disabled:opacity-50">{seccionCargando === "regenerar-objeciones" ? "ÔÅ│ Generando..." : "Ôå╗ Regenerar"}</button>
                            <button disabled={seccionCargando !== null} onClick={() => mejorar("objeciones")} className="bg-cyan-400/10 border border-cyan-400/20 text-cyan-400 text-[10px] font-bold px-2 py-1 rounded-md disabled:opacity-50">{seccionCargando === "mejorar-objeciones" ? "ÔÅ│ Mejorando..." : "Ôåæ Mejorar"}</button>
                            <button onClick={() => copiar(resultado.objeciones, "objeciones")} className="bg-[#111] border border-[#1e1e1e] text-zinc-400 text-[10px] font-bold px-2 py-1 rounded-md">{copiado === "objeciones" ? "Ô£ô Copiado" : "Copiar"}</button>
                            <button onClick={() => guardar(resultado.objeciones, "Objeciones")} className="bg-green-400/10 border border-green-400/20 text-green-400 text-[10px] font-bold px-2 py-1 rounded-md">ÔØñ Guardar</button>
                          </div>
                        </div>
                    ...
                    {resultado.email && (
                      <div className="border border-zinc-600 rounded-xl overflow-hidden">
                        <div className="flex items-center justify-between px-4 py-2.5 border-b border-zinc-600/20">
                          <span className="text-zinc-400 text-[10px] font-bold tracking-widest uppercase">­ƒôº Email de seguimiento</span>
                          <div className="flex gap-1.5">
                            <button disabled={seccionCargando !== null} onClick={() => regenerar("email")} className="bg-orange-500/10 border border-orange-500/25 text-orange-500 text-[10px] font-bold px-2 py-1 rounded-md disabled:opacity-50">{seccionCargando === "regenerar-email" ? "ÔÅ│ Generando..." : "Ôå╗ Regenerar"}</button>
                            <button disabled={seccionCargando !== null} onClick={() => mejorar("email")} className="bg-cyan-400/10 border border-cyan-400/20 text-cyan-400 text-[10px] font-bold px-2 py-1 rounded-md disabled:opacity-50">{seccionCargando === "mejorar-email" ? "ÔÅ│ Mejorando..." : "Ôåæ Mejorar"}</button>
                            <button onClick={() => copiar(resultado.email, "email")} className="bg-[#111] border border-[#1e1e1e] text-zinc-400 text-[10px] font-bold px-2 py-1 rounded-md">{copiado === "email" ? "Ô£ô Copiado" : "Copiar"}</button>
                            <button onClick={() => guardar(resultado.email, "Email")} className="bg-green-400/10 border border-green-400/20 text-green-400 text-[10px] font-bold px-2 py-1 rounded-md">ÔØñ Guardar</button>
                          </div>
                        </div>
                ...
                {guardados.length > 0 && (
                  <div className="border border-orange-500 rounded-xl overflow-hidden">
                    <div className="px-4 py-2.5 border-b border-orange-500/20 flex items-center justify-between">
                      <span className="text-orange-500 text-[10px] font-bold tracking-widest uppercase">ÔØñ Mis copys guardados ({guardados.length})</span>
                        ...
                        <div key={i} className="py-3 border-b border-[#111] last:border-none">
                          <div className="flex items-center justify-between mb-1">
                            <div className="flex items-center gap-2">
                              ...
                              <span className="text-zinc-600 text-[10px]">┬À</span>
                              <span className="text-cyan-400 text-[10px] font-bold">{g.tipo}</span>
                            </div>
                            <div className="flex items-center gap-2">
                              <span className="text-zinc-600 text-[10px]">{g.hora}</span>
                              <button onClick={() => copiar(g.texto, `guardado-${i}`)} className="bg-[#111] border border-[#1e1e1e] text-zinc-400 text-[10px] px-2 py-1 rounded-md">{copiado === `guardado-${i}` ? "Ô£ô Copiado" : "Copiar"}</button>
                              <button onClick={async () => { if (g.id) { await fetch("/api/copys", { method: "DELETE", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ id: g.id }) }); } setGuardados(prev => prev.filter((_, idx) => idx !== i)); }} className="text-red-400 text-[10px] font-bold px-1">Ô£ò</button>
                              {g.id && <button onClick={() => { navigator.clipboard.writeText(`${window.location.origin}/share/${g.id}`); setCopiado(`share-${i}`); setTimeout(() => setCopiado(null), 2000); }} className="bg-purple-500/10 border border-purple-500/20 text-purple-400 text-[10px] font-bold px-2 py-1 rounded-md">{copiado === `share-${i}` ? "Ô£ô Link copiado" : "­ƒöù Compartir"}</button>}
                            </div>
                          </div>
                          <p onClick={() => setExpandido(expandido === i ? null : i)} className="text-[#f0ead6] text-xs cursor-pointer hover:text-white transition-colors whitespace-pre-wrap">
                            ...
                            <span className="text-zinc-500 ml-1">{expandido === i ? "Ôû▓ ver menos" : "Ôû╝ ver m├ís"}</span>
                          </p>
                        </div>
                      ))}
                    </div>
